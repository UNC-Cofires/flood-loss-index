...
CONTAINS
    ...
    FUNCTION steady_state_diffusion(Nx,Ny,dx,dy,C0,Nbc,bc_coords,bc_values,Nconv,conv_coords,tol) RESULT(C)
    
        ! This function computes the steady-state solution to the 2-D diffusion equation
        ! using the Jacobi iteration method. Sources and sinks are specified based on
        ! the dirichlet boundary condition coordinates and values supplied by the user. 
        ! Edges of the computational domain are subject to a no-flux boundary condition.  

        INTEGER, INTENT(IN) :: Nx, Ny, Nbc, Nconv
        REAL(KIND = 8), INTENT(IN) :: dx, dy, tol
        ...
        REAL(KIND = 8), DIMENSION(Ny,Nx) :: C
        REAL(KIND = 8), DIMENSION(Ny,Nx) :: C_prev
        REAL(KIND = 8), DIMENSION(Nconv) :: diff
        REAL(KIND = 8) :: dx2, dy2, x_mult, y_mult, max_diff
        INTEGER :: i, j, idx, iter_num
        ...
        DO WHILE (max_diff >= tol)
            ...
            DO j = 2,Nx-1
                DO i = 2, Ny-1
                    ...
                    C(i,j) = x_mult*(C_prev(i,j+1) + C_prev(i,j-1)) + y_mult*(C_prev(i+1,j) + C_prev(i-1,j))
