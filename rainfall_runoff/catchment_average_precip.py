...
def weighted_average_precip(precip,timepoints,weightmap,weight_col,point_col):
    """
    ...
    param: weightmap: pandas groupby object describing overlap between catchment polygons and gridded precipitation data.
                    The group keys should correspond to catchment polygon identifiers (e.g., COMID for NHDPlus). 
    ...
    catchment_id_list = list(weightmap.groups.keys())
    ...
    for j,catchment_id in enumerate(catchment_id_list):
        ...
        point_indices = weightmap.get_group(catchment_id)[point_col].to_numpy()
        point_weights = weightmap.get_group(catchment_id)[weight_col].to_numpy()
...
outfolder = os.path.join(aggregation_folder,f'AORC_by_COMID')
...
# Turn weightmap into pandas groupby object so that we can 
# quickly look up the gridcells associated with each COMID
weightmap = weightmap.groupby('COMID')
...
period_list = pd.period_range(start=f'{min_year}-01', end=f'{max_year}-12',freq='Y')
...
for period in period_list:
    ...
    catchment_precip = weighted_average_precip(precip,timepoints,weightmap,'overlap_area','point_index')
